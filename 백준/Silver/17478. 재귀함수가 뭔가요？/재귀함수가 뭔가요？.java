import java.io.BufferedReader;
import java.io.InputStreamReader;

public class Main {
    static String intro = "어느 한 컴퓨터공학과 학생이 유명한 교수님을 찾아가 물었다.\n";
    static String answer = "\"재귀함수가 뭔가요?\"\n";
    static String endStr = "라고 답변하였지.\n";
    static String result = "\"재귀함수는 자기 자신을 호출하는 함수라네\"\n";
    static String depthStr = "____";
    static String recur1 = "\"잘 들어보게. 옛날옛날 한 산 꼭대기에 이세상 모든 지식을 통달한 선인이 있었어.\n";
    static String recur2 = "마을 사람들은 모두 그 선인에게 수많은 질문을 했고, 모두 지혜롭게 대답해 주었지.\n";
    static String recur3 = "그의 답은 대부분 옳았다고 하네. 그런데 어느 날, 그 선인에게 한 선비가 찾아와서 물었어.\"\n";
    static StringBuilder sb = new StringBuilder();

    public static void main(String[] args) throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        int depth = Integer.parseInt(br.readLine());
        sb.append(intro);
        recursive(0, depth);
        System.out.println(sb);
    }

    private static void recursive(int depth, int end) {
        if (depth == end) {
            sb.append(makeLine(depth)).append(answer);
            sb.append(makeLine(depth)).append(result);
            sb.append(makeLine(depth)).append(endStr);
            return;
        }
        sb.append(makeLine(depth)).append(answer);
        sb.append(makeLine(depth)).append(recur1);
        sb.append(makeLine(depth)).append(recur2);
        sb.append(makeLine(depth)).append(recur3);
        recursive(depth + 1, end);
        sb.append(makeLine(depth)).append(endStr);
    }

    private static String makeLine(int depth) {
        StringBuilder s = new StringBuilder();
        for (int i = 0; i < depth; i++) {
            s.append(depthStr);
        }
        return s.toString();
    }
}